...
function ValueCell({ value, onChange }: { value: number; onChange: (value: number) => Promise<void> }) {
...
export function AccountDealsTable({
  deals,
  reps,
  leadCountByDeal
}: {
  deals: Deal[];
  reps: Rep[];
  leadCountByDeal: Map<string, number>;
}) {
  if (deals.length === 0) {
    return <p className="text-sm text-neutral-500">No deals linked to this account yet — link one from the Deals page.</p>;
  }
