...
// Shared by the scheduled cron route and the manual "Sync Now" button, so there's a
// reliable fallback when the Vercel cron scheduler doesn't fire on its own.
...
export async function runInstantlySync() {
  ...
  return { ok: true, inserted, progressUpdated, skippedCampaigns, repliesProcessed };
}
